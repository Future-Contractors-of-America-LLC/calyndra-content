# Art quality QC report

Generated: **2026-06-24 12:18 UTC**

**Overall:** PASS (0 issue(s), 8 warning(s))

## Manifest band folders (ship gate)

| Band | PNGs OK | Manifest words | Status |
|------|---------|----------------|--------|
| seed (bootstrap) | 146 | 146 | PASS |
| sprout | 146 | 146 | PASS |
| bud | 146 | 146 | PASS |
| sprig | 146 | 146 | PASS |
| vine | 146 | 146 | PASS |
| bloom | 146 | 146 | PASS |
| canopy | 146 | 146 | PASS |

## Expanded vocabulary coverage (informational)

| Audience | PNGs OK | Total symbols | Status |
|----------|---------|---------------|--------|
| baby | 40 | 55 | WARN |
| toddler | 66 | 88 | WARN |
| child | 66 | 120 | WARN |
| tween | 66 | 187 | WARN |
| teen | 66 | 230 | WARN |
| adult | 69 | 245 | WARN |
| caregiver | 38 | 58 | WARN |

## Dimensions

- Pass (readable): **1218** PNG(s)
- Fail (out of range): **0**
- Target: **512x512** (warnings if different)

## Manifest

- Band-unique words: **146**
- Placeholder words: **0**
- Bootstrap bands: **1**

## Sidecar metadata

- Reviewed sidecars: **259**
- Bad handFingers: **0**

## Issues

- None

## Warnings

- VOCAB: `baby` missing 15/55 imageAsset PNG(s).
- VOCAB: `toddler` missing 22/88 imageAsset PNG(s).
- VOCAB: `child` missing 54/120 imageAsset PNG(s).
- VOCAB: `tween` missing 121/187 imageAsset PNG(s).
- VOCAB: `teen` missing 164/230 imageAsset PNG(s).
- VOCAB: `adult` missing 176/245 imageAsset PNG(s).
- VOCAB: `caregiver` missing 20/58 imageAsset PNG(s).
- META: 35 hand-visible symbol(s) lack sidecar metadata (optional until art review).
