# Continuous Ultra HD (3840x2160) Caly and Friends production with per-episode ship.
$root = "C:\Users\MichaelBartholomew\Documents\GitHub"
$content = Join-Path $root "calyndra-content"
$stamp = Get-Date -Format "yyyyMMdd-HHmmss"
$log = Join-Path $content "videos\uhd_render_$stamp.log"
$env:PYTHONUNBUFFERED = "1"
$env:CALY_VIDEO_PROFILE = "uhd"

. (Join-Path $content "scripts\render_ship_utils.ps1")

function Test-EpisodeReadyToShip {
  param([string]$Id)
...
for e in c['episodes']:
    ...
    webm = app / e['webm']
    ok = (
        e.get('videoProfile') == 'uhd'
        and webm.is_file()
        and meets_duration_target(e, webm)
    )
    print('yes' if ok else 'no')
    break
"@
  return ($ready -match 'yes')
}
...
for e in c['episodes']:
    if e.get('status') != 'complete':
        continue
    webm = app / e['webm']
    if e.get('videoProfile') == 'uhd' and webm.is_file() and meets_duration_target(e, webm):
        continue
    upgrade.append(e['id'])
...
function Render-One {
  ...
  if (-not (Test-EpisodeReadyToShip -Id $Id)) {
    Write-RenderLog "SKIP ship: $Id not UHD-complete (duration or profile)" $log
    return $false
  }
  ...
  Write-RenderLog "Shipped Ultra HD $Id -> web + mobile + CDN deploy" $log
  return $true
}

Write-RenderLog "=== UHD continuous production start (3840x2160 @ 24fps) ===" $log
Set-Location $content

python -m pip install --quiet imageio-ffmpeg pillow 2>&1 | Out-Null
$ffmpeg = python -c "import imageio_ffmpeg; print(imageio_ffmpeg.get_ffmpeg_exe())" 2>&1
if (-not (Test-Path $ffmpeg)) {
  Write-RenderLog "ERROR: ffmpeg not available" $log
  exit 1
}
Write-RenderLog "ffmpeg: $ffmpeg" $log
...
while ($true) {
  ...
  if ($pendingCount -eq 0 -and $upgradeCount -eq 0) {
    Write-RenderLog "All episodes complete at Ultra HD." $log
    break
  }
...
Write-RenderLog "=== UHD continuous production done - log: $log ===" $log
